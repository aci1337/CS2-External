...
std::wstring GenerateRandomClassName()
{
    ...
    std::wstring randomClassName;

    for (int i = 0; i < length; ++i)
    {
        int randomIndex = std::rand() % (wcslen(charset));

        randomClassName += charset[randomIndex];
    }
...
namespace OSImGui
{
    ...
    void OSImGui_External::NewWindow(std::string A, Vec2 B, std::function<void()> C)
    {
        if (!C)
    ...
    void OSImGui_External::AttachAnotherWindow(std::string D, std::string E, std::function<void()> F)
    {
        if (!F)
            throw OSException("Error");
        ...
        DestWindow.hWnd = FindWindowA(
            ...
            (D.empty() ? NULL : D.c_str()));

        if (DestWindow.hWnd == NULL)
            throw OSException("Error");

        DestWindow.Name = D;
        DestWindow.ClassName = E;
    ...
    {
        std::wstring className = GenerateRandomClassName();

        WNDCLASSEXW wc = { sizeof(wc), CS_CLASSDC, WndProc_External, 0L, 0L, GetModuleHandle(NULL), NULL, NULL, NULL, NULL, className.c_str(), NULL };
        RegisterClassExW(&wc);

        if (Type == ATTACH)
        {
            // Create a transparent topmost window with less conspicuous attributes.
            Window.hWnd = CreateWindowExW(WS_EX_LAYERED | WS_EX_TOPMOST | WS_EX_TRANSPARENT,
                className.c_str(), L"", WS_POPUP, CW_USEDEFAULT, CW_USEDEFAULT, 100, 100, NULL, NULL, GetModuleHandle(NULL), NULL);
            SetLayeredWindowAttributes(Window.hWnd, 0, 255, LWA_ALPHA);
        }
        else
        {
            Window.hWnd = CreateWindowW(className.c_str(), L"", WS_OVERLAPPED | WS_MINIMIZEBOX | WS_SYSMENU,
                (int)Window.Pos.x, (int)Window.Pos.y, (int)Window.Size.x, (int)Window.Size.y, NULL, NULL, wc.hInstance, NULL);
        }
